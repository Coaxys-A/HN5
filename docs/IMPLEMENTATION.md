...
- Admin router (`src/admin/routes.js`) now wraps critical SQL strings in a Base64 decoding layer, enforces hostname locking, and records audits with obfuscated action identifiers.
...
## Tooling & DevOps
- `package.json` describes build, lint, test, obfuscation, and seed scripts with required dependencies; the obfuscation step delegates to `obfuscate.js` to emit signed manifests.
...
## Secret handling
- `.env.secure` (documented in `docs/README_SECURE.md`) loads before runtime defaults so operators can inject Base64-protected secrets outside the repository.
