...
const {
  login, logout, ensureAuthenticated, recordAudit, requireRole, ERROR_CODES,
} = require('./auth');
const { createSetupSecret, verifySetupToken, hasVerifiedTotp } = require('./totp');
...
const CONTENT_STATUS = ['visible', 'incoming', 'coming_soon', 'preparing', 'hidden'];
const KNOWN_LOGIN_ERRORS = new Set(Object.values(ERROR_CODES));
...
const QUERIES = {
  ...
  classInsert:
    'INSERT INTO classes (title, slug, schedule, description, capacity, active, status) VALUES (?, ?, ?, ?, ?, ?, ?)',
  classUpdate:
    'UPDATE classes SET title=?, slug=?, schedule=?, description=?, capacity=?, active=?, status=? WHERE id=?',
  programSelect: 'SELECT * FROM programs ORDER BY created_at DESC',
  programInsert: 'INSERT INTO programs (title, description, visible, status) VALUES (?, ?, ?, ?)',
  programUpdate: 'UPDATE programs SET title=?, description=?, visible=?, status=? WHERE id=?',
  articleSelect: 'SELECT * FROM articles ORDER BY published_at DESC, created_at DESC',
  articleInsert:
    'INSERT INTO articles (slug, title, excerpt, content, read_more_url, author_id, published_at, tags, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
  articleUpdate:
    'UPDATE articles SET slug=?, title=?, excerpt=?, content=?, read_more_url=?, published_at=?, tags=?, status=? WHERE id=?',
...
function normalizeStatus(value) {
  ...
  return CONTENT_STATUS.includes(value) ? value : 'visible';
}
...
router.post('/login', csrfProtection, async (req, res) => {
  try {
    ...
    await recordAudit(result.user.id, req.ip, 'login', { user: result.user.username });
    res.json({ [RESPONSE.success]: true, user: result.user, security: result.security });
  } catch (error) {
    const statusCode =
      error.message === ERROR_CODES.twoFactorRequired || error.message === ERROR_CODES.invalidTwoFactor ? 401 : 400;
    const payloadError = KNOWN_LOGIN_ERRORS.has(error.message) ? error.message : 'login_failed';
    res.status(statusCode).json({ [RESPONSE.error]: payloadError });
  }
});
...
router.post('/2fa/setup', ensureAuthenticated, allowSuperAdmin, csrfProtection, async (req, res) => {
  if (!config.ENABLE_2FA) {
    return res.status(404).json({ [RESPONSE.error]: STATUS.disabled });
  }
  const enrollment = await createSetupSecret(req.session.user);
  await recordAudit(req.session.user.id, req.ip, '2fa.setup', {});
  return res.json({
    [RESPONSE.success]: true,
    secret: enrollment.secret,
    otpauth_url: enrollment.otpauthUrl,
    qr: enrollment.qrCode,
  });
});
...
router.post(
  ...
  async (req, res) => {
    ...
    const verified = await verifySetupToken(req.session.user.id, req.body.token);
    if (!verified) {
      return res.status(400).json({ [RESPONSE.error]: STATUS.invalidToken });
    }
    await recordAudit(req.session.user.id, req.ip, '2fa.enable', {});
    return res.json({ [RESPONSE.success]: true });
  },
);

router.get('/2fa/status', ensureAuthenticated, allowSuperAdmin, async (req, res) => {
  if (!config.ENABLE_2FA) {
    return res.json({ enabled: false });
  }
  const enabled = await hasVerifiedTotp(req.session.user.id);
  return res.json({ enabled });
});

router.get('/session', ensureAuthenticated, async (req, res) => {
  const twoFactorEnforced =
    config.ENABLE_2FA && req.session.user.role === 'absolute_developer'
      ? await hasVerifiedTotp(req.session.user.id)
      : false;
  res.json({ user: req.session.user, security: { twoFactorEnforced } });
});
...
const articleValidators = [
  ...
  body('status').optional({ checkFalsy: true }).isIn(CONTENT_STATUS),
];

async function scanUploadedFile(filePath, mimeType) {
  try {
    await fs.promises.access(filePath, fs.constants.R_OK);
    console.info(`[uploads] queued for antivirus scan: ${filePath} (${mimeType})`);
  } catch (error) {
    console.warn('[uploads] antivirus hook failed to access file', error);
  }
}
...
router.post(
  ...
  if (payload.id) {
    await pool.query(QUERIES.programUpdate, [title, description, visible, status, payload.id]);
    await recordAudit(req.session.user.id, req.ip, 'programs.update', { id: payload.id });
  } else {
    const [result] = await pool.query(QUERIES.programInsert, [title, description, visible, status]);
...
router.post(
  ...
  async (req, res) => {
    ...
    await scanUploadedFile(req.file.path, req.file.mimetype);
